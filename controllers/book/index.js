...
// method: POST
// url: /api/book
// !NAME OF FILE MUST BE IN FORMAT OF "{TITLE} - {AUTHOR}.{EXTENSION}
export const createBook = async (req, res) => {
  try {
    const file = req?.file;
    if (!file) throw new CustomError("No book was sent");

    const [title, author] = file?.originalname
      .replace(/\.(epub|zip)$/, "")
      .split(" - ");
    const titleInUrl = title?.replace(/[-_,()\s]+/g, "+");
    const authorInUrl = author?.replace(/[-_,()\s]+/g, "+");

    let coverId = "";
    try {
      const bookDetails = await fetch(
        `https://openlibrary.org/search.json?title=${titleInUrl}&author=${authorInUrl}`
      );

      if (bookDetails.ok) {
        const bookDetailsJson = await bookDetails.json();
        if (bookDetailsJson?.docs.length) {
          coverId =
            bookDetailsJson?.docs[0]?.cover_i || "/book-default-cover.jpg";
        }
      }
    } catch (error) {
      console.error("Failed to fetch book cover ID", error);
    }
